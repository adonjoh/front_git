...
export default function Statistiques() {
    ...
    const StatCard = ({ title, value, icon: Icon, colorClass, bgColorClass, description }) => (
        <div className="bg-white/[0.02] backdrop-blur-md rounded-xl border p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
            ...
            {description && (
                <p className="text-sm text-slate-400 relative z-10 border-t border-white/5 pt-3 mt-3">
                    {description}
                </p>
            )}
        </div>
            ...
            <div className="space-y-6">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        {/* Section Statuts */}
                        <h2 className="text-xl font-bold text-white mt-10 mb-4">Statuts des comptes</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-white/[0.02] backdrop-blur-md rounded-xl border p-6 shadow-sm flex items-center">
                                <div className="p-4 bg-yellow-100 rounded-full mr-6">
                                    <Clock className="w-8 h-8 text-yellow-600" />
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-slate-400 uppercase tracking-wider">En Attente de Validation</p>
                                    <div className="flex items-baseline mt-1">
                                        <span className="text-4xl font-extrabold text-white">{stats?.en_attente !== undefined ? stats.en_attente : '-'}</span>
                                        <span className="ml-2 text-sm text-slate-400">comptes</span>
                                    </div>
                                    <p className="text-sm text-amber-400 mt-2 font-medium">Nécessite votre attention dans l'onglet Inscriptions</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white/[0.02] backdrop-blur-md rounded-xl border p-6 shadow-sm flex items-center">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-slate-400 uppercase tracking-wider">Comptes Suspendus</p>
                                    <div className="flex items-baseline mt-1">
                                        <span className="text-4xl font-extrabold text-white">{stats?.suspendus !== undefined ? stats.suspendus : '-'}</span>
                                        <span className="ml-2 text-sm text-slate-400">comptes</span>
                                    </div>
                                    <p className="text-sm text-rose-400 mt-2 font-medium">Interdits d'accès par le Censeur ou l'Administrateur</p>
                                </div>
                            </div>
                        </div>

                        {/* Note supplémentaire si besoin */}
                        <div className="mt-8 bg-indigo-500/10 rounded-xl p-5 border border-indigo-100 flex items-start">
