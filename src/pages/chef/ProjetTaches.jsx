        ...
        <DashboardLayout title="Gestion des Tâches">
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
                    <div>
                        <Link to="/mes-projets" className="inline-flex items-center text-sm text-gray-500 hover:text-indigo-600 mb-2 transition-colors">
                ...
                {showForm && (
                    <div className="bg-white p-6 rounded-xl border border-indigo-100 shadow-md">
                        <h2 className="text-lg font-bold text-gray-900 mb-4">Créer une nouvelle tâche</h2>
                        <form onSubmit={handleCreateTask} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">ID du membre assigné (Optionnel)</label>
                                <input
                                    type="number"
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"
                ...
                {loading ? (
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {tasks.map(task => (
                            <div key={task.id} className={`bg-white rounded-xl border p-5 shadow-sm transition-all ${task.statut === 'soumis' ? 'border-yellow-300 bg-yellow-50/30' : ''}`}>
                                ...
                                <h3 className="text-lg font-bold text-gray-900 mb-2">{task.titre}</h3>
                                <p className="text-gray-600 text-sm mb-4 line-clamp-3">{task.description}</p>
                                
                                {task.assignee && (
                                    <div className="text-sm text-gray-500 mb-4 bg-gray-50 px-3 py-2 rounded-lg">
                                        Assigné à : <span className="font-medium text-gray-900">{task.assignee.name}</span>
                                    </div>
                                )}
