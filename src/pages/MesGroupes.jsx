...
export default function MesGroupes() {
    ...
    return (
        <DashboardLayout title="Mes Groupes de Discussion">
            <div className="bg-white rounded-2xl border shadow-sm overflow-hidden h-[calc(100vh-140px)] flex">
                
                {/* Liste des groupes (Sidebar) */}
                <div className={`${activeGroup ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-80 border-r border-gray-200 bg-gray-50 h-full`}>
                                ...
                                {groupes.map(groupe => (
                                    <li key={groupe.id}>
                                        <button
                                            ...
                                            className={`w-full text-left px-4 py-4 hover:bg-indigo-50 transition-colors flex items-center gap-3 ${activeGroup?.id === groupe.id ? 'bg-indigo-50 border-l-4 border-indigo-600' : 'border-l-4 border-transparent'}`}
                                        >
                                            <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-indigo-600 font-bold flex-shrink-0 shadow-sm">
                                                <Hash className="w-5 h-5" />
                                            </div>
                                            <div className="overflow-hidden flex-1">
                                                <h3 className="font-bold text-gray-900 truncate">{groupe.nom}</h3>
                                                <p className="text-xs text-gray-500 truncate">{groupe.description || 'Groupe de discussion'}</p>
                                            </div>
                                        </button>
                                    </li>
                                ))}
                ...
                {/* Zone de Chat */}
                <div className={`${!activeGroup ? 'hidden md:flex' : 'flex'} flex-col flex-1 h-full bg-white`}>
                    {!activeGroup ? (
                                    ...
                                    messages.map((msg, index) => {
                                        ...
                                        return (
                                            <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                                {showHeader && !isMe && (
                                                    <span className="text-xs font-medium text-gray-500 mb-1 ml-1">{msg.user?.name || 'Inconnu'}</span>
                                                )}
                                                <div className={`max-w-[75%] px-4 py-2 rounded-2xl relative ${isMe ? 'bg-indigo-600 text-white rounded-tr-sm' : 'bg-white border border-gray-200 text-gray-800 rounded-tl-sm shadow-sm'}`}>
                            ...
                            <div className="p-4 bg-white border-t border-gray-200">
                                <form onSubmit={handleSendMessage} className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 bg-gray-100 border-transparent focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 rounded-full px-5 py-2.5 outline-none transition-all"
                                        value={newMessage}
