...
export default function Scores() {
    ...
    const getNiveauInfo = (score) => {
        if (score >= 2000) return { nom: 'Élite', couleur: 'bg-purple-100 text-purple-800 border-purple-200', icon: <Trophy className="w-6 h-6 text-purple-600" /> };
        if (score >= 1001) return { nom: 'Expert', couleur: 'bg-red-100 text-red-800 border-red-200', icon: <Award className="w-6 h-6 text-red-600" /> };
        ...
        if (score >= 201) return { nom: 'Membre', couleur: 'bg-blue-100 text-blue-800 border-blue-200', icon: <Star className="w-6 h-6 text-blue-600" /> };
        return { nom: 'Recrue', couleur: 'bg-gray-100 text-gray-800 border-gray-200', icon: <Target className="w-6 h-6 text-gray-600" /> };
    };
        ...
        <DashboardLayout title="Scores et Classement">
            <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                    {/* Mon Score */}
                    <div className="bg-white rounded-xl border p-6 shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-4xl font-extrabold text-indigo-600 mb-1">{scoreTotal} <span className="text-lg font-medium text-gray-500">pts</span></p>
                        ...
                        {badges.length === 0 ? (
                            ...
                            <div className="flex flex-wrap gap-3">
                                {badges.map((badge, idx) => (
                                    <div key={idx} className="flex flex-col items-center p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-indigo-200 transition-colors" title={badge.description}>
                                        {/* Assuming badge might have an icon or we use a generic one */}
                                        <div className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center mb-2">
                                            <span className="text-2xl">{badge.icon || '🏆'}</span>
                                        </div>
                                        <span className="text-xs font-medium text-center text-gray-800">{badge.nom}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-white border-b border-gray-100">
                                        <th className="p-4 font-semibold text-gray-600 text-sm w-16 text-center">Rang</th>
                                        <th className="p-4 font-semibold text-gray-600 text-sm">Membre</th>
                                        <th className="p-4 font-semibold text-gray-600 text-sm">Niveau</th>
                                        <th className="p-4 font-semibold text-gray-600 text-sm text-right">Score Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {classement.map((membre, index) => {
                                        ...
                                        return (
                                            <tr key={membre.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${isCurrentUser ? 'bg-indigo-50/50' : ''}`}>
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center">
                                                        ...
                                                        <div>
                                                            <div className={`font-semibold ${isCurrentUser ? 'text-indigo-700' : 'text-gray-900'}`}>
                                                                {membre.name} {isCurrentUser && <span className="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded ml-2">Vous</span>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
